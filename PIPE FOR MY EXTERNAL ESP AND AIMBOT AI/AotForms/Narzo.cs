...

{
    public class NARZO
    {
        ...
        private async Task<IEnumerable<long>> AobScan(string pattern)
        {
            ...
            bool foundFirst = false; // Flag to stop after first match
            ...
            await Task.Run(delegate
            {
                ...
                // Sequential search instead of parallel to control order and stop at first match
                foreach (MemoryPage addresss in list.OrderBy(p => (long)p.Start))
                {
                    if (foundFirst) break; // Stop if we already found one
                    ...
                    {
                        int num = FindPattern(array, patternData.pattern, patternData.mask, 0);
                        if (num >= 0)
                        {
                            long foundAddress = (long)addresss.Start + num;
                            lock (addressRet)
                            {
                                if (!addressRet.Contains(foundAddress))
                                {
                                    addressRet.Add(foundAddress);
                                    foundFirst = true; // Mark as found, stop searching
                                }
                            }
                        }
                    }
        ...
        {
            // Even more restrictive filtering to match Cheat Engine behavior
            if (page.State == 4096) // MEM_COMMIT only
            {
                // Only allow specific memory types that Cheat Engine typically scans
                if (page.Type == 131072) // MEM_PRIVATE
                {
                    return page.Protect == 4; // PAGE_READWRITE only for private memory
                }
                else if (page.Type == 16777216) // MEM_IMAGE (executable modules)
                {
                    return page.Protect == 32 || page.Protect == 64; // PAGE_EXECUTE_READ || PAGE_EXECUTE_READWRITE
                }
            }
            return false;
        }
